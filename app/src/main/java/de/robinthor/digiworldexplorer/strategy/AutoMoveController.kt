...
object AutoMoveController{
 ...
 fun onAnalysis(confidence:Double,bounds:GridBounds,cells:Map<Cell,CellScores>,preview:Map<Cell,CellScores>){
  ...
  val obstacles=cells.filter{(c,s)->c!=player&&s.obstacle()}.keys+preview.filter{it.value.obstacle()}.keys;val items=cells.filter{(c,s)->c!=player&&s.item>.06}.keys+preview.filter{it.value.item>.06}.keys
  ...
  val action=MovementPlanner.choose(player!!,cells,history.toList(),false,preview);val status=if(AutomationState.enabled)"AUTO: ${action?.kind?:"STOP"} ${action?.reason?:"keine Route"}" else "PAUSE: Automatik aus"
