...
class DigiWorldAccessibilityService:AccessibilityService(){
 ...
 inner class GridOverlayView:View(this){var bounds:GridBounds?=null;var player:Cell?=null;var items:Set<Cell> = emptySet();var obstacles:Set<Cell> = emptySet();var target:Cell?=null;var status="Analyse";var captureMode=false;private val p=Paint(Paint.ANTI_ALIAS_FLAG).apply{style=Paint.Style.STROKE;strokeWidth=4f;textSize=30f}
  ...
  override fun onDraw(c:Canvas){super.onDraw(c);val b=bounds?:return;val cw=(b.right-b.left)/5f;val ch=(b.bottom-b.top)/5f;p.color=Color.GREEN;p.style=Paint.Style.STROKE;for(i in 0..5){c.drawLine(b.left+i*cw,b.top.toFloat(),b.left+i*cw,b.bottom.toFloat(),p);c.drawLine(b.left.toFloat(),b.top+i*ch,b.right.toFloat(),b.top+i*ch,p)}
   p.pathEffect=DashPathEffect(floatArrayOf(12f,8f),0f);c.drawRect(b.right.toFloat(),b.top.toFloat(),b.right+cw,b.bottom.toFloat(),p);for(i in 1..4)c.drawLine(b.right.toFloat(),b.top+i*ch,b.right+cw,b.top+i*ch,p);p.pathEffect=null;if(captureMode)return
