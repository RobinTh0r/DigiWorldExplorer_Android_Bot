...
class DigiWorldAccessibilityService:AccessibilityService(){
 ...
 fun hideForCapture(){overlay?.post{overlay?.visibility=View.INVISIBLE}}
 fun updateOverlay(bounds:GridBounds?,player:Cell?,items:Set<Cell>,obstacles:Set<Cell>,target:Cell?,status:String,visible:Boolean){overlay?.post{overlay?.apply{this.bounds=bounds;this.player=player;this.items=items;this.obstacles=obstacles;this.target=target;this.status=status;visibility=if(visible)View.VISIBLE else View.GONE;invalidate()}}}
