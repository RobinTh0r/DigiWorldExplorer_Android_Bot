...
class MainActivity:ComponentActivity(){
 ...
 override fun onCreate(savedInstanceState:Bundle?){super.onCreate(savedInstanceState);AutomationState.stop();if(Build.VERSION.SDK_INT>=33&&ContextCompat.checkSelfPermission(this,Manifest.permission.POST_NOTIFICATIONS)!=PackageManager.PERMISSION_GRANTED)notifications.launch(Manifest.permission.POST_NOTIFICATIONS);setContent{MaterialTheme{Surface(Modifier.fillMaxSize()){ControlScreen(status,captureActive,autoActive,{startActivity(Intent(Settings.ACTION_ACCESSIBILITY_SETTINGS))},{startActivity(Intent(Settings.ACTION_MANAGE_OVERLAY_PERMISSION,Uri.parse("package:$packageName")))},{consent.launch(getSystemService(MediaProjectionManager::class.java).createScreenCaptureIntent())},{if(captureActive){ScreenCaptureService.setAutomation(this,true);autoActive=true;status="AUTOMATIK AKTIV – Stopp jederzeit möglich"}},{ScreenCaptureService.setAutomation(this,false);autoActive=false;status="Analyse aktiv – Automatik gestoppt"},{ScreenCaptureService.stop(this);captureActive=false;autoActive=false;status="Alles gestoppt"})}}}}
}
@Composable private fun ControlScreen(status:String,capture:Boolean,auto:Boolean,onAccess:()->Unit,onOverlay:()->Unit,onCapture:()->Unit,onAuto:()->Unit,onAutoStop:()->Unit,onAllStop:()->Unit){Column(Modifier.fillMaxSize().padding(24.dp),verticalArrangement=Arrangement.Center){Text("DigiWorld Explorer",style=MaterialTheme.typography.headlineMedium);Spacer(Modifier.height(14.dp));Card(Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp)){Text("Status",style=MaterialTheme.typography.labelLarge);Text(status);Text("Raster-Overlay: aktiv")}};Spacer(Modifier.height(16.dp));Button(onClick=onCapture,Modifier.fillMaxWidth()){Text(if(capture)"Bildschirmfreigabe erneuern" else "1. Bildschirmfreigabe starten")};OutlinedButton(onClick=onAccess,Modifier.fillMaxWidth()){Text("2. Bedienungshilfe öffnen")};OutlinedButton(onClick=onOverlay,Modifier.fillMaxWidth()){Text("3. Raster über anderen Apps erlauben")};Spacer(Modifier.height(10.dp));Button(onClick=onAuto,enabled=capture&&!auto,modifier=Modifier.fillMaxWidth(),colors=ButtonDefaults.buttonColors(containerColor=MaterialTheme.colorScheme.tertiary)){Text("4. AUTOMATIK STARTEN")};Button(onClick=onAutoStop,enabled=capture,modifier=Modifier.fillMaxWidth(),colors=ButtonDefaults.buttonColors(containerColor=MaterialTheme.colorScheme.error)){Text("AUTOMATIK SOFORT STOPPEN")};OutlinedButton(onClick=onAllStop,Modifier.fillMaxWidth()){Text("Alles stoppen")};Spacer(Modifier.height(10.dp));Text("Auch über die Android-Benachrichtigung stoppbar. Wegwischen der App stoppt die Automatik.",style=MaterialTheme.typography.bodySmall)}}