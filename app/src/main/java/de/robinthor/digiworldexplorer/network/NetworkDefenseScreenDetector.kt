...
object NetworkDefenseScreenDetector {
    fun detect(width: Int, height: Int, argbAt: (Int, Int) -> Int): NetworkDefenseDetection {
        ...
        // Tall phones place the 1/2 ticket badge lower and farther right than emulator layouts.
        val startTicketPurple = ratio(width, height, .70, .09, .96, .22, argbAt, ::purple)
        val attemptButtonCenter = centerOfMatch(width, height, .50, .54, .84, .62, argbAt, ::cyan)
